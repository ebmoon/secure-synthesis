...
class SynthSpec extends AnyFunSpec {
  ...
  describe("Synth") {
    ...
    it("should synthesize correct conditional (if x then y = 5 else y = 3)") {
      val inputType = Map("x" -> RefType(BooleanType(Low), High), "y" -> RefType(IntType(High), High))
      val goalType = UnitType
      val example1 = new Example(
        preCxt = Map("x" -> LocValue(100), "y" -> LocValue(101)),
    ...
    it("should fail to synthesize conditional (if x then y = 5 else y = 3)") {
      val inputType = Map("x" -> RefType(BooleanType(High), High), "y" -> RefType(IntType(Low), High))
      val goalType = UnitType
      val example1 = new Example(
        preCxt = Map("x" -> LocValue(100), "y" -> LocValue(101)),
        preEnv = Map(100 -> BooleanValue(True), 101 -> NumValue(0)),
